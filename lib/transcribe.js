...
function formatWithSpeakers(text, speakerSegments) {
  ...
  sentences.forEach((sentence, index) => {
    // Detectar cambios de hablante por:
    // 1. Preguntas seguidas de respuestas
    // 2. Cada 3-4 oraciones (conversación natural)
    // 3. Cambios de tono implícitos (preguntas vs afirmaciones)
    
    const isQuestion = sentence.trim().endsWith('?');
    const prevWasQuestion = index > 0 && sentences[index - 1].trim().endsWith('?');
    
    // Si la oración anterior era pregunta y esta no, probablemente cambió el hablante
    if (prevWasQuestion && !isQuestion) {
      currentSpeaker = currentSpeaker === 1 ? 2 : 1;
    }
    
    // Cambiar hablante cada 3-5 oraciones para simular diálogo
    if (index > 0 && index % 4 === 0 && !prevWasQuestion) {
      currentSpeaker = currentSpeaker === 1 ? 2 : 1;
    }
